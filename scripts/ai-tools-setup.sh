...
#   - claude-code-router — routes background/subtask calls to cheaper models
#
...
# ── Step 7: claude-code-router ────────────────────────────────────────────────
if command -v ccr &>/dev/null; then
  info "claude-code-router already installed."
else
  info "Installing claude-code-router (routes background tasks to cheaper models)..."
  npm install -g @musistudio/claude-code-router
fi

# Deploy config template (skip if user already has a config with a real key)
CCR_CONFIG="${HOME}/.claude-code-router/config.json"
TEMPLATE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/configs/claude-code-router/config.json"

if [[ -f "$CCR_CONFIG" ]] && ! grep -q "ANTHROPIC_API_KEY_PLACEHOLDER" "$CCR_CONFIG" 2>/dev/null; then
  info "claude-code-router config already configured: $CCR_CONFIG"
else
  mkdir -p "$(dirname "$CCR_CONFIG")"
  if [[ -n "${ANTHROPIC_API_KEY:-}" ]]; then
    sed "s/ANTHROPIC_API_KEY_PLACEHOLDER/${ANTHROPIC_API_KEY}/" "$TEMPLATE" > "$CCR_CONFIG"
    info "claude-code-router config written with ANTHROPIC_API_KEY."
  else
    cp "$TEMPLATE" "$CCR_CONFIG"
    warn "ANTHROPIC_API_KEY not set — edit $CCR_CONFIG and replace ANTHROPIC_API_KEY_PLACEHOLDER."
  fi
fi

# Write ccr-service wrapper so launchd/systemd can start it without fnm on PATH
CCR_SERVICE="${HOME}/.local/bin/ccr-service"
mkdir -p "$(dirname "$CCR_SERVICE")"
cat > "$CCR_SERVICE" << 'EOF'
#!/usr/bin/env bash
# Auto-generated by perdev ai-tools-setup.sh
eval "$(fnm env --shell bash 2>/dev/null)" || true
exec ccr start
EOF
chmod +x "$CCR_SERVICE"
info "ccr-service wrapper written to $CCR_SERVICE"
...
info ""
info "  Start token-efficient Claude Code (RTK + ccr routing):"
info "    just ccr-code              # start router + open Claude Code"
info "    just ccr-start             # start router only (background)"
info ""
